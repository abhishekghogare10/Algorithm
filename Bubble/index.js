    ...
    for (let j = 0; j < array.length - i - 1; j++) {
      if (array[j] > array[j + 1]) {
        for (let k = 0; k < bars.length; k++) {
          if (k !== j && k !== j + 1) {
            bars[k].style.backgroundColor = "rgb(67, 255, 249)";
          }
        }
        let temp = array[j];
        array[j] = array[j + 1];
        ...
        bars[j + 1].style.backgroundColor = "violet";
...
async function quickSort(items, left, right) {
  ...
  for (let i = 0; i < bars.length; i++) {
    bars[i].style.backgroundColor = "rgb(67, 255, 249)";
  }
  return items;
}
